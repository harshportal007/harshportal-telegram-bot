...
async function safeReply(ctx, text, extra = {}) {
  try { return await ctx.reply(text, { parse_mode: 'HTML', disable_web_page_preview: true, ...extra }); }
  catch (e) { console.error('Reply error:', e); }
}
async function notifyAdmin(bot, text, extra = {}) {
  try { return await bot.telegram.sendMessage(ADMIN_ID, text, { parse_mode: 'HTML', ...extra }); }
  catch (e) { console.error('Admin notify error:', e); }
}
function reportSupaError(ctx, cmd, error, bot) {
  console.error(`${cmd} error:`, error);
  safeReply(ctx, `❌ Error fetching ${cmd.replace('/', '')}.`);
  notifyAdmin(bot, `❌ <b>${cmd}</b> failed:\n<pre>${html(error?.message || JSON.stringify(error))}</pre>`);
}

/** ---------- singleton bot (so Vercel reuses it across invocations) ---------- */
let _bot;
function getBot() {
  if (_bot) return _bot;
...
module.exports = async (req, res) => {
  ...
  // Command list visible in Telegram
  ...
  bot.command('reply', async (ctx) => {
    ...
    if (!userId || !replyMsg) return safeReply(ctx, 'Usage: <code>/reply &lt;user_id&gt; &lt;message&gt;</code>');
    try {
